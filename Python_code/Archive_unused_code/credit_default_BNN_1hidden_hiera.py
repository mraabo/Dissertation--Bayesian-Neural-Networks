...
def construct_bnn(ann_input, ann_output, n_hidden = 5):
    ...
    with pm.Model() as bayesian_neural_network:
        ...
        weights_in_1 = pm.Normal("w_in_1", mu=mu1, sigma=sigma1, shape=(X_train.shape[1], n_hidden), testval=init_1)
        ...
        weights_1_out = pm.Normal("w_1_out", mu=mu_out, sigma=sigma_out, shape=(n_hidden), testval=init_out)
...
with bayesian_neural_network_NUTS:
    trace = pm.sample(draws=3000, tune=1000, chains=1, target_accept=.90)
