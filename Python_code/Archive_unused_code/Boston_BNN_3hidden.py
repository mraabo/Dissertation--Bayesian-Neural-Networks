...
def construct_bnn(ann_input, ann_output, n_hidden = 5, prior_std = 0.01):
    ...
    init_2 = np.random.randn(n_hidden, n_hidden).astype(floatX)*prior_std
    init_3 = np.random.randn(n_hidden, n_hidden).astype(floatX)*prior_std
    ...
    # Layer 1 -> Layer 2
        weights_2 = pm.Normal('w_2', mu=0, sd=prior_std,
                          shape=(n_hidden, n_hidden),
                          testval=init_2)
        acts_2 = pm.Deterministic('activations_2', tt.nnet.relu(tt.dot(acts_1, weights_2)))

    # Layer 2 -> Layer 3
        weights_3 = pm.Normal('w_3', mu=0, sd=prior_std,
                            shape=(n_hidden,n_hidden),
                            testval=init_3)
        acts_3= pm.Deterministic('activations_3',tt.nnet.relu(tt.dot(acts_2, weights_3)))
        
    # Layer 3 -> Output Layer
        weights_out = pm.Normal('w_out', mu=0, sd=prior_std,
                            shape=(n_hidden,1),
                            testval=init_out)
        acts_out = pm.Deterministic('activations_out',tt.dot(acts_3, weights_out))
...
bayesian_neural_network_NUTS = construct_bnn(X_train, y_train, n_hidden=15, prior_std=0.1)
